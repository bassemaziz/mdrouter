...
async def test_search_finds_content(doc_store):
    ...
    # Rebuild FTS
    await doc_store._store.execute(
        "INSERT INTO docs_pages_fts(docs_pages_fts) VALUES('rebuild')"
    )
...
async def test_search_scoped_to_source(doc_store):
    ...
    await doc_store.upsert_page(
        source1["id"], "https://one.com/1", "Python Guide", "Python programming"
    )
    await doc_store.upsert_page(
        source2["id"], "https://two.com/1", "Rust Guide", "Rust programming"
    )
    await doc_store._store.execute(
        "INSERT INTO docs_pages_fts(docs_pages_fts) VALUES('rebuild')"
    )
...
async def test_has_summaries_false(doc_store):
    source = await doc_store.add_source("test", "https://test.com")
    page_id, _ = await doc_store.upsert_page(
        source["id"], "https://test.com/1", "P", "c"
    )
...
async def test_save_and_get_summaries(doc_store):
    ...
    await doc_store.save_summary(
        page_id, 0, "chunk text", "summary text", "deepseek/deepseek-chat", 150
    )
